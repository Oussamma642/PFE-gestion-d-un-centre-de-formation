...
const StudentGradeReport = ({
    ...
    const calculateAverages = () => {
        ...
        modules.forEach((module) => {
            ...
            const controleAvg = calculateAverageForStudent(
        ...
        const controleAvg =
            controleCount > 0 ? controleTotal / controleCount : 0;
        const theoriqueAvg =
            theoriqueCount > 0 ? theoriqueTotal / theoriqueCount : 0;
        const pratiqueAvg =
            pratiqueCount > 0 ? pratiqueTotal / pratiqueCount : 0;
        const generalAvg =
    ...
    function determineDecision(moyenneGenerale) {
        return moyenneGenerale >= 10 ? "Admis" : "Non admis";
    }
    ...
    function calculateMoyennGenerale(
        moyGeneraleCC,
        moyGeneraleExT,
        moyGeneraleExP
    ) {
        return (
            (moyGeneraleCC * 3 + moyGeneraleExT * 2 + moyGeneraleExP * 3) /
            8
        ).toFixed(2);
    }
                        ...
                        {modules.map((module) => {
                            ...
                            return (
                                <tr key={module.id}>
                                    ...
                                    <td className="border border-gray-300 p-2 text-center">
                                        {pratiqueGrade !== null &&
                                        pratiqueGrade !== undefined ? (
                                            <span
                                                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                    parseFloat(pratiqueGrade) >=
                                                    10
                                                        ? "bg-green-100 text-green-800"
                                                        : "bg-red-100 text-red-800"
                                                }`}
                            ...
                            <tr>
                                ...
                                <td
                                    className={`border border-gray-300 p-2 text-center font-bold ${
                                        parseFloat(
                                            // Calculate Moyenn des notes pour CC, ExT, ExP
                                ...
                                >
                                    {
                                        ...
                                        // using the weighted average of (moyGeneraleCC * 3 + moyGeneraleExT * 2 + moyGeneraleExP * 3) / 8
                                    calculateMoyennGenerale(
                                        calculateMoynneCCEXs(
                                            sommeMoyCC,
                                            sommeCoeffModules
                        ...
                        <tr className="bg-gray-200">
                            ...
                            <td
                                className={`border border-gray-300 p-2 text-center font-medium ${
                                    // Function to determine the decision based on the average
                                    determineDecision(
                                        calculateMoyennGenerale(
                                            calculateMoynneCCEXs(
                                                sommeMoyCC,
