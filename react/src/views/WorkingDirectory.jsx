...
const WorkingDirectory = () => {
  ...
  useEffect(() => {
    const fetchFilieres = async () => {
      ...
      try {
        ...
        setError("Impossible de charger les filières. Veuillez réessayer plus tard.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const options = [
    ...
    {
      id: 4,
      title: "Passage de la première année à la deuxième année",
      description: "Gérez le passage des étudiants",
      icon: <ArrowRightCircle className="text-purple-600" />,
      color: "bg-purple-50 border-purple-200",
      hoverColor: "hover:bg-purple-100",
      iconBg: "bg-purple-100",
      hasFiliere: true,
  ...
  const handleSubOptionClick = (subOptionId) => {
    if (!selectedFiliere && selectedOption === 1) return;
    switch (subOptionId) {
      case 1:
        navigate(`/dashboard/filiere/${selectedFiliere}/annee/premiere_annee`);
        break;
      case 2:
        navigate(`/dashboard/filiere/${selectedFiliere}/annee/deuxieme_annee`);
        break;
      default:
        navigate(`/dashboard/filiere/${selectedFiliere}/annee/premiere_annee`);
    }
            ...
            >
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h2 className="text-xl font-semibold text-slate-800">{option.title}</h2>
                      <p className="text-sm text-slate-600 mt-1">{option.description}</p>
                    </div>
                  </div>
                  {(option.subOptions.length > 0 || option.hasFiliere) && (
                    <ChevronDown
              ...
              {selectedOption === option.id && (
                ...
                >
                  {option.hasFiliere && (
                    <div className="p-4">
                      <h3 className="text-md font-medium text-slate-700 mb-3">Sélectionnez une filière:</h3>
                      {loading ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                          {filieres.map((filiere) => (
                            ...
                            >
                              <span>{filiere.libelle}</span>
                              {selectedFiliere === filiere.id && <span className="text-blue-600 font-bold">✔</span>}
                            </motion.button>
                          ))}
                        </div>
                      )}
                  ...
                  {option.subOptions.length > 0 && (!option.hasFiliere || selectedFiliere) && (
                    <div className="p-4 space-y-1">
                      {option.subOptions.map((subOption) => (
                        ...
                        >
                          <span className="text-slate-700 font-medium">{subOption.title}</span>
                          {subOption.icon}
                        </motion.button>
                      ))}
                    </div>
